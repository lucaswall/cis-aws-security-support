...
class SecuritySupportStack extends cdk.Stack {
  constructor(scope, id, props) {
    ...
    const guardDutyFindingsToSlackLambda = new lambda.Function(this, 'GuardDutyFindingsToSlack', {
      runtime: lambda.Runtime.NODEJS_12_X,
      handler: 'index.handler',
      code: lambda.Code.fromAsset('lambda-GuardDutyFindingsToSlack'),
      environment: {
        SLACK_PREFIX: ssm.StringParameter.valueForStringParameter(this, 'SecuritySupport_SlackPrefix'),
        SLACK_WEBHOOKPATH: ssm.StringParameter.valueForStringParameter(this, 'SecuritySupport_SlackWebHook'),
      }
    ...
    new events.Rule(this, 'GuardDutyFindingsToSlackRule', {
      ruleName: 'GuardDutyFindingsToSlackRule',
      eventPattern: {
        source: ['aws.guardduty'],
      },
      targets: [new eventstargets.LambdaFunction(guardDutyFindingsToSlackLambda)],
    });
    ...
    const cloudTrailAlarmList = [
      ...
      { id: 'CIS-3.5-CloudTrailChanges', filter: '{($.eventName=CreateTrail) || ($.eventName=UpdateTrail) || ($.eventName=DeleteTrail) || ($.eventName=StartLogging) || ($.eventName=StopLogging)}' },
      { id: 'CIS-3.6-ConsoleAuthenticationFailure', filter: '{($.eventName=ConsoleLogin) && ($.errorMessage="Failed authentication")}' },
    ...
    new iam.ManagedPolicy(this, 'HealthReadOnly', {
